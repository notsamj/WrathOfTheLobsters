...
class SmokeCloud {
    ...
    */
    static create(x, y){
        let circles = [];
        let numCircles = randomNumberInclusive(RETRO_GAME_DATA["smoke_generation"]["min_circles_per_smoke_cloud"], RETRO_GAME_DATA["smoke_generation"]["max_circles_per_smoke_cloud"]);
        let mainRadius = RETRO_GAME_DATA["smoke_generation"]["center_radius"];
        ...
        for (let i = 0; i < numCircles; i++){
            ...
            let circleRadius = randomNumberInclusive(RETRO_GAME_DATA["smoke_generation"]["min_radius"], RETRO_GAME_DATA["smoke_generation"]["max_radius"]);
            let circleLifeLength = randomNumberInclusive(RETRO_GAME_DATA["smoke_generation"]["min_life_ms"], RETRO_GAME_DATA["smoke_generation"]["max_life_ms"]);
            let xVelocity = randomFloatBetween(RETRO_GAME_DATA["smoke_generation"]["max_speed"] * -1, RETRO_GAME_DATA["smoke_generation"]["max_speed"]);
            let yVelocity = randomFloatBetween(RETRO_GAME_DATA["smoke_generation"]["max_speed"] * -1, RETRO_GAME_DATA["smoke_generation"]["max_speed"]);
